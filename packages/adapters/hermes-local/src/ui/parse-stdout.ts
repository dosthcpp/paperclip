 ...
 */
function parseToolCompletionLine(line: string): ToolCompletionInfo | null {
  ...
  const hasError =
    /\[(?:exit \d+|error|full)\]/.test(verbAndDetail) || /\[error\]\s*$/.test(cleaned);
...
export function parseHermesStdoutLine(line: string, ts: string): TranscriptEntry[] {
  ...
  if (trimmed.includes(TOOL_OUTPUT_PREFIX)) {
    ...
    const stripped = trimmed
      .replace(/^\[done\]\s*/, "")
      .replace(
        new RegExp(`^${TOOL_OUTPUT_PREFIX.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*`),
        "",
      )
      .trim();
